// A complete, valid, entirely synthetic environment for config tests.
// Values are placeholders; none of them are real credentials.
export const SECRET_TOKEN_VALUE = 'token-synthetic-7f3a9c1e';
export const CRM_KEY_VALUE = 'crm-key-synthetic-0123456789abcdef-0123';
/** base64 of 32 synthetic bytes (0x01 x 32). */
export const MFA_KEY_VALUE = Buffer.alloc(32, 1).toString('base64');

/**
 * An absolute path that exists on neither platform's disk. loadConfig only
 * checks the SHAPE of MEDIA_ROOT (set, and absolute); the folder itself is
 * checked at start-up by media/root.ts, so these tests never touch a disk.
 */
export const MEDIA_ROOT_VALUE =
  process.platform === 'win32' ? 'C:\\academy-media-synthetic' : '/srv/academy-media-synthetic';

export function validEnv(): Record<string, string> {
  return {
    NODE_ENV: 'test',
    DB_HOST: 'localhost',
    DB_PORT: '5432',
    DB_NAME: 'academy_test',
    DB_USER: 'academy_app',
    DB_PASSWORD: 'db-password-synthetic',
    ACADEMY_V2: 'false',
    STAGE1_AUTH_REQUIRED: 'false',
    CRM_AUTH_URL: 'http://localhost:3000/api/auth/academy-verify',
    CRM_AUTH_KEY: CRM_KEY_VALUE,
    MFA_ENCRYPTION_KEY: MFA_KEY_VALUE,
    MEDIA_ROOT: MEDIA_ROOT_VALUE,
    PUBLIC_BASE_URL: 'http://localhost:5173',
  };
}

/** A valid production environment: https everywhere, Redis, no mock CRM. */
export function validProdEnv(): Record<string, string> {
  return {
    ...validEnv(),
    NODE_ENV: 'production',
    REDIS_URL: 'redis://localhost:6379',
    CRM_AUTH_URL: 'https://crm.example.invalid/api/auth/academy-verify',
    PUBLIC_BASE_URL: 'https://academy.example.invalid',
  };
}

export const REQUIRED_VARS = [
  'DB_HOST',
  'DB_PORT',
  'DB_NAME',
  'DB_USER',
  'DB_PASSWORD',
  'ACADEMY_V2',
  'STAGE1_AUTH_REQUIRED',
  'CRM_AUTH_URL',
  'CRM_AUTH_KEY',
  'MFA_ENCRYPTION_KEY',
  'MEDIA_ROOT',
  'PUBLIC_BASE_URL',
] as const;
