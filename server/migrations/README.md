...
Later migrations: `0003_seed_support.sql` (S02: DEPARTMENT recording category, department metadata, `stages.sort`, question upsert key) and `0004_auth_support.sql` (S03: `trainees.track` optional, `sessions.sid_hash`). `0006_notifications.sql` (S08: `notifications_sent`, the UNIQUE key that makes "3 fails → ONE message" survive a restart, a retry and two workers racing). Every new table must also grant `academy_app` its rights in the same migration.
