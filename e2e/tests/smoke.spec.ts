...
//
// Neither test needs a single row of training content, so both carry
// CONTENT_FREE_TAG and run on a CI runner's empty database.

test('the app is up and the flag is on', { tag: CONTENT_FREE_TAG }, async ({ request }) => {
...
test(
  'a signed-out visitor is sent to sign in, and the API refuses them',
